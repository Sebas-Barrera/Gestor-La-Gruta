    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4 pt-2">
          <FormField label="Nombre del Almacén" required error={errors.name}>
            <TouchInput value={name} onChange={(e) => setName(e.target.value)} placeholder="Ej: Almacén Central" />
          </FormField>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button onClick={handleSubmit} className="flex-1 bg-blue-500 hover:bg-blue-600">
              {isEditing ? 'Guardar Cambios' : 'Agregar Almacén'}
            </Button>
          </div>
        </div>
      </DialogContent>
